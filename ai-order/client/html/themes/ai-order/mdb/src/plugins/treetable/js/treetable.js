...
class Treetable {
  ...
  _setArrowsPositionInRow(rows) {
    rows.forEach((row) => {
      if (SelectorEngine.find(SELECTOR_BUTTON, row)) {
        SelectorEngine.find(SELECTOR_BUTTON, row).innerHTML = expandingArrow.rightArrow;
      }
    });
  }

  _setArrowPosition(nestedRows, firstRows, element) {
  ...
  _collapse(nestedRows, firstRows, expandablesFromFirstNest, element) {
    if (!nestedRows.every((x) => x.classList.contains('hidden'))) {
      ...
      this._setArrowPosition(nestedRows, firstRows, element);
      this._setArrowsPositionInRow(nestedRows);
...
SelectorEngine.find(SELECTOR_TREETABLE).forEach((treetable) => {
